...
- (void)closeRecordingWithCompletion:(VideoCompletionBlock)completionBlock {
    if (_isRecording) {
        _isRecording = NO;
        [_displayLink removeFromRunLoop:[NSRunLoop mainRunLoop] forMode:NSRunLoopCommonModes];
        [self closeRecordingSession:completionBlock];
    }
    else {
        completionBlock();
    }
...
- (void)closeRecordingSession:(VideoCompletionBlock)completionBlock {
    dispatch_async(_render_queue, ^{
        dispatch_sync(_append_pixelBuffer_queue, ^{
